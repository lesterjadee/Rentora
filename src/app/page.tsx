  ...
  return (
    <>
      <style>{`
        ...
        /* QR TOP BANNER */
        .hp-qr-top {
          background: linear-gradient(135deg, #011E05, #023D09 50%, #011E05);
          border-bottom: 1px solid rgba(6,214,33,0.1);
          padding: 14px 28px;
          display: flex; align-items: center; justify-content: center; gap: 24px;
          flex-wrap: wrap;
        }
        .hp-qr-top::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, rgba(201,168,76,0.22), transparent); }

        /* HERO */
        .hp-hero { position: relative; padding: 100px 28px 110px; overflow: hidden; }
        ...
        .hp-overline-pill { display: inline-flex; align-items: center; gap: 10px; padding: 7px 16px 7px 8px; background: rgba(4,149,22,0.07); border: 1px solid rgba(4,149,22,0.18); border-radius: 999px; margin-bottom: 28px; }
        ...
        .hp-card { background: #FFFFFF; border: 1.5px solid rgba(4,149,22,0.18); border-radius: 24px; padding: 26px; box-shadow: 0 20px 60px rgba(1,30,5,0.13), 0 4px 16px rgba(1,30,5,0.07); position: relative; overflow: hidden; }
        ...
        .hp-card-img { background: var(--bg-raised); border: 1px solid rgba(4,149,22,0.08); border-radius: 16px; height: 165px; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; overflow: hidden; }
        ...
        .hp-step {
          padding: 44px 36px;
          border-right: 1px solid rgba(4,149,22,0.1);
        ...
        .hp-step-n {
          ...
          letter-spacing: -0.06em; line-height: 1; margin-bottom: 16px;
          color: var(--g-rich);
          opacity: 0.22;
        }
        .hp-step-bar { width: 32px; height: 4px; border-radius: 999px; margin-bottom: 20px; }
        .hp-step h3 {
        ...
        .hp-step p {
          font-size: 14px; color: var(--tx-body);
        ...
        /* RESPONSIVE */
        @media (max-width: 1024px) { .hp-hero-inner { grid-template-columns: 1fr; gap: 56px; } .hp-trust-grid { grid-template-columns: 1fr; gap: 48px; } .hp-features-grid-wrap { grid-template-columns: 1fr; } .hp-features-sticky { position: static; } }
        @media (max-width: 900px)  { .hp-feats { grid-template-columns: 1fr 1fr; } }
        @media (max-width: 768px)  { .hp-steps-box { grid-template-columns: 1fr; } .hp-step { border-right: none; border-bottom: 1px solid rgba(4,149,22,0.1); } .hp-step:last-child { border-bottom: none; } .hp-hero { padding: 80px 20px; } .hp-section { padding: 72px 20px; } .hp-qr-top { padding: 12px 20px; gap: 16px; } }
      ...
      <div className="hp">
        ...
        {/* ── QR CODE TOP BANNER ── */}
        <div className="hp-qr-top" style={{ position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <div style={{ background: '#FFFFFF', borderRadius: '12px', padding: '8px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
              <img
                src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=https://rentora-tau-flame.vercel.app/&color=023D09&bgcolor=ffffff&qzone=1&format=png"
                alt="QR Code" width={80} height={80}
                style={{ display: 'block', borderRadius: '6px' }}
              />
            </div>
            <div>
              <p style={{ fontSize: '14px', fontWeight: '800', color: '#F0FFF2', margin: '0 0 4px', letterSpacing: '-0.01em' }}>
                📱 Scan to open Rentora on your phone
              </p>
              <p style={{ fontSize: '12px', color: 'rgba(240,255,242,0.6)', margin: '0 0 6px' }}>
            ...
            {!user && (
              <div style={{ display: 'flex', gap: '10px', marginLeft: '8px' }}>
                <Link href="/auth/register" className="btn-gold" style={{ fontSize: '13px', padding: '9px 20px' }}>
        ...
        <section className="hp-hero">
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 70% 60% at 65% -5%, rgba(4,149,22,0.09) 0%, transparent 60%), radial-gradient(ellipse 50% 40% at 5% 80%, rgba(2,61,9,0.06) 0%, transparent 50%)' }} />
        ...
        <section className="hp-section">
          <div className="hp-inner">
            ...
            <div className="hp-steps-box">
              {[
                { n: '01', color: 'var(--g-mid)', shadow: 'rgba(4,149,22,0.4)', title: 'Create your account', desc: 'Sign up with your @gordoncollege.edu.ph email. Verification is instant and automatic.' },
                { n: '02', color: 'var(--au-mid)', shadow: 'rgba(201,168,76,0.4)', title: 'Browse or list items', desc: 'Find what you need or list your own items to earn from them while they sit unused.' },
                { n: '03', color: 'var(--g-mid)', shadow: 'rgba(4,149,22,0.4)', title: 'Rent with confidence', desc: 'Request, agree on dates, and transact safely within the Gordon College community.' },
              ].map((step, i) => (
                <div key={i} className="hp-step">
                  <div className="hp-step-n">{step.n}</div>
                  <div className="hp-step-bar" style={{ background: step.color, boxShadow: `0 0 12px ${step.shadow}` }} />
                  <h3>{step.title}</h3>
                  <p>{step.desc}</p>
                </div>
              ))}
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                {user ? (
                  ...
                  <>
                    ...
                    <Link href="/auth/login" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '13px 26px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', color: 'rgba(240,255,242,0.8)', fontWeight: '600', fontSize: '14px', borderRadius: '12px', textDecoration: 'none' }}>Sign in</Link>
                  </>
                )}
              </div>
            </div>
