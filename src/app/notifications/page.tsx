...
export default async function NotificationsPage() {
  ...
  const typeConfig: Record<string, NotifConfig> = {
    rental_request: {
      ...
      bg: 'rgba(59,130,246,0.08)', border: 'rgba(59,130,246,0.18)', color: '#1D4ED8', label: 'New Request'
    },
    rental_approved: {
      iconEl: <CheckCircle size={18} color="#2ECC8F" strokeWidth={1.8} />,
      bg: 'var(--g-glow)', border: 'rgba(34,168,118,0.2)', color: '#037312', label: 'Approved'
    },
    rental_declined: {
      iconEl: <XCircle size={18} color="#FCA5A5" strokeWidth={1.8} />,
      bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.18)', color: '#B91C1C', label: 'Declined'
    },
    rental_completed: {
      iconEl: <Flag size={18} color="#E2C07A" strokeWidth={1.8} />,
      bg: 'var(--au-glow)', border: 'rgba(201,168,76,0.2)', color: '#5A3F14', label: 'Completed'
    },
    new_review: {
      iconEl: <Star size={18} color="#C9A84C" strokeWidth={1.8} fill="#C9A84C" />,
      bg: 'var(--au-glow)', border: 'rgba(201,168,76,0.2)', color: '#5A3F14', label: 'New Review'
    },
  }
  ...
  return (
    <>
      <style>{`
        ...
        .notif-item.unread { border-color: rgba(27,77,62,0.2); }
        .notif-icon-box {
        ...
        <div className="notif-inner">
          {notifications && notifications.length > 0 ? (
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px', flexWrap: 'wrap' as const, gap: '6px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' as const }}>
                          <p style={{ fontWeight: '800', fontSize: '14px', color: '#071A09', margin: 0, letterSpacing: '-0.01em' }}>{notif.title}</p>
                      ...
                      <p style={{ fontSize: '13px', color: '#1A3020', margin: '0 0 6px', lineHeight: '1.5' }}>{notif.message}</p>
                      <p style={{ fontSize: '11px', color: '#4E7055', margin: 0, fontWeight: '600' }}>
            ...
            <div className="notif-empty">
              ...
              <p style={{ fontWeight: '800', fontSize: '20px', color: '#071A09', marginBottom: '8px', letterSpacing: '-0.03em' }}>All caught up!</p>
              <p style={{ fontSize: '14px', color: '#1A3020' }}>No notifications yet. Start renting to see activity here.</p>
            </div>
          )}
        </div>
      </div>
