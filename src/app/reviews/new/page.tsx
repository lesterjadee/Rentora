...
function ReviewForm() {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '12px', color: 'var(--tx-muted)', margin: 0 }}>{description}</p>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '24px', fontWeight: '900', color: barColor, lineHeight: 1 }}>
              {value}<span style={{ fontSize: '14px', color: 'var(--tx-muted)', fontWeight: '600' }}>/10</span>
            </div>
            <div style={{ fontSize: '11px', fontWeight: '700', color: barColor, marginTop: '4px', textTransform: 'uppercase' as const, letterSpacing: '0.05em' }}>{getLabel(value)}</div>
          </div>
        </div>

        {/* Progress bar */}
        <div style={{ height: '6px', background: 'var(--bg-hover)', borderRadius: '999px', marginBottom: '16px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${percentage}%`, backgroundColor: barColor, borderRadius: '999px', transition: 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)', boxShadow: `0 0 10px ${barColor}60` }} />
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  backgroundColor: isActive ? `${barColor}15` : '#FFFFFF',
                  color: isActive ? barColor : 'var(--tx-body)',
  ...
  if (!rental) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-void)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: 'var(--tx-muted)', fontSize: '14px', fontWeight: '600', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>Loading rental...</p>
    </div>
  )
    ...
    <>
      <style>{`
        ...
        .rev-textarea {
          ...
          background: var(--bg-raised); border: 1.5px solid rgba(27,77,62,0.1);
        ...
        .btn-submit {
          ...
          border: 1px solid rgba(92,219,149,0.15); color: #FFFFFF;
          cursor: pointer; transition: all 0.25s;
          box-shadow: 0 4px 20px rgba(7,24,18,0.2);
          font-family: 'Plus Jakarta Sans', system-ui, sans-serif;
        }
        .btn-submit:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 24px rgba(7,24,18,0.3); }
        ...
        .error-box {
          ...
          border-radius: 12px; padding: 14px 16px; color: #B91C1C;
          font-size: 13px; font-weight: 600; margin-bottom: 24px;
        }
      `}</style>

      <div className="rev-page">
        {/* Header Banner — matches other pages */}
        <div className="rev-banner" style={{ background: 'linear-gradient(150deg, #011E05 0%, #023D09 40%, #02560E 100%)' }}>
          <div style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '16px', position: 'relative' }}>
            <Link href={`/rentals/${rentalId}`} style={{
              width: '38px', height: '38px',
              background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.14)',
              borderRadius: '11px', display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: 'rgba(240,255,242,0.75)', textDecoration: 'none', flexShrink: 0, transition: 'all 0.2s',
            }}>
              <ArrowLeft size={17} strokeWidth={2} />
            </Link>
            <div>
              <p style={{ fontSize: '11px', fontWeight: '800', color: '#7FFFC4', letterSpacing: '0.12em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>Rate Your Experience</p>
              <h1 style={{ fontSize: 'clamp(24px,5vw,32px)', fontWeight: '900', color: '#F0FDF4', margin: 0, letterSpacing: '-0.03em' }}>Leave a Review</h1>
            </div>
          </div>
        </div>
        ...
        <div className="rev-inner">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="rev-card">
              ...
              {/* Overall Score Preview — light card, dark text */}
              <div className="score-preview">
                <p style={{ fontSize: '11px', fontWeight: '800', color: 'var(--tx-muted)', textTransform: 'uppercase' as const, letterSpacing: '0.1em', margin: '0 0 16px' }}>Overall Score Preview</p>

                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  <div style={{ flex: 1, height: '8px', background: 'var(--bg-hover)', borderRadius: '999px', overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', width: `${overallPercentage}%`,
                      background: 'linear-gradient(90deg, var(--g-rich), #2ECC8F)',
                      borderRadius: '999px', transition: 'width 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                      boxShadow: '0 0 10px rgba(46,204,143,0.3)',
                    }} />
                  </div>
                  <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                    <span style={{ fontSize: '32px', fontWeight: '900', color: 'var(--g-mid)' }}>{overallPercentage}</span>
                    <span style={{ fontSize: '16px', color: 'var(--tx-muted)', fontWeight: '700' }}>%</span>
                  </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid rgba(27,77,62,0.1)' }}>
                  <span style={{ fontSize: '12px', color: 'var(--tx-muted)', fontWeight: '600' }}>Average of 3 categories</span>
                  <div style={{ background: 'rgba(201,168,76,0.1)', padding: '6px 14px', borderRadius: '8px', border: '1px solid rgba(201,168,76,0.25)' }}>
                    <span style={{ fontSize: '12px', fontWeight: '800', color: 'var(--au-dark)' }}>Trust Score: {overallRating}/5</span>
                  </div>
                </div>
              </div>
            </div>
