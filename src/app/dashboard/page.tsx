...
export default async function DashboardPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f0f4f8', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {/* Top banner */}
      <div style={{
        background: 'linear-gradient(135deg, #1a3a5c 0%, #26619C 50%, #3b82f6 100%)',
        padding: '40px 24px 80px'
      }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <p style={{ fontSize: '13px', fontWeight: '600', color: 'rgba(255,255,255,0.6)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>
              Dashboard
            </p>
            <h1 style={{ fontSize: '36px', fontWeight: '800', color: '#ffffff', letterSpacing: '-0.02em', margin: '0 0 8px' }}>
          ...
          {[
            {
              ...
              color: '#26619C', lightColor: '#eff6ff', icon: '📦',
              gradient: 'linear-gradient(135deg, #26619C, #3b82f6)'
            },
            {
              label: 'Active Rentals', value: myRentals?.length || 0,
              color: '#0891b2', lightColor: '#ecfeff', icon: '🔄',
              gradient: 'linear-gradient(135deg, #0891b2, #06b6d4)'
            },
            {
              label: 'Trust Score', value: profile?.trust_score || '--',
              color: '#d97706', lightColor: '#fffbeb', icon: '⭐',
              gradient: 'linear-gradient(135deg, #d97706, #f59e0b)'
            },
          ].map((stat, i) => (
            <div key={i} style={{
              backgroundColor: '#ffffff', borderRadius: '20px',
              border: '1px solid #e8edf2', padding: '28px',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <div style={{
                  width: '40px', height: '40px', background: stat.gradient,
          ...
          {[
            ...
            <Link key={i} href={link.href} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                boxShadow: `0 8px 24px ${link.shadow}`,
