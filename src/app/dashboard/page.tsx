      ...
      <div className="dash">
        ...
        <div style={{ maxWidth: '1280px', margin: '-64px auto 0', padding: '0 28px 60px' }}>
          ...
          <div className="dash-links">
            {[
              { href: '/items', label: 'Browse Items', desc: 'Find items to rent', icon: <ShoppingBag size={22} strokeWidth={1.8} />, iconColor: '#FFFFFF', iconBg: 'rgba(255,255,255,0.15)', bg: 'linear-gradient(135deg, #071812, #0D2B20, #1B4D3E)', border: 'rgba(92,219,149,0.15)', shadow: '0 6px 20px rgba(7,24,18,0.2)', arrowColor: 'rgba(127,255,196,0.6)' },
              { href: '/items/new', label: 'List an Item', desc: 'Earn from your stuff', icon: <PlusCircle size={22} strokeWidth={1.8} />, iconColor: '#92400E', iconBg: 'rgba(201,168,76,0.18)', bg: 'linear-gradient(135deg, #fffbeb, #fef3c7)', border: 'rgba(201,168,76,0.3)', shadow: '0 6px 20px rgba(201,168,76,0.12)', arrowColor: 'var(--au-dark)' },
              { href: '/rentals', label: 'My Rentals', desc: 'Track your rentals', icon: <ClipboardList size={22} strokeWidth={1.8} />, iconColor: '#1D4ED8', iconBg: 'rgba(59,130,246,0.12)', bg: 'linear-gradient(135deg, #eff6ff, #dbeafe)', border: 'rgba(59,130,246,0.25)', shadow: '0 6px 20px rgba(59,130,246,0.10)', arrowColor: '#1D4ED8' },
              { href: '/recommendations', label: 'For You', desc: 'Personalized picks', icon: <Sparkles size={22} strokeWidth={1.8} />, iconColor: '#6D28D9', iconBg: 'rgba(124,58,237,0.12)', bg: 'linear-gradient(135deg, #f5f3ff, #ede9fe)', border: 'rgba(124,58,237,0.2)', shadow: '0 6px 20px rgba(124,58,237,0.10)', arrowColor: '#6D28D9' },
            ].map((link, i) => (
              <Link key={i} href={link.href} className="dash-link-card" style={{ background: link.bg, border: `1.5px solid ${link.border}`, boxShadow: link.shadow }}>
                ...
                <p style={{ fontWeight: '800', fontSize: '14px', color: i === 0 ? '#F0FDF4' : 'var(--tx-bright)', margin: '0 0 4px', letterSpacing: '-0.01em' }}>{link.label}</p>
                <p style={{ fontSize: '12px', color: i === 0 ? 'rgba(240,253,244,0.6)' : 'var(--tx-muted)', margin: 0 }}>{link.desc}</p>
              </Link>
            ))}
          </div>
