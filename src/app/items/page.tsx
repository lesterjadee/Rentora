    ...
    <>
      <style>{`
        ...
        .browse-input {
          ...
          font-size: 14px; color: #F0FFF2 !important; outline: none;
        ...
        .browse-select {
          ...
          color: #F0FFF2 !important; cursor: pointer; min-width: 160px;
          font-family: 'Plus Jakarta Sans', system-ui, sans-serif;
        }
        ...
        .browse-clear { display: flex; align-items: center; gap: 6px; padding: 11px 16px; background: rgba(239,68,68,0.14); border: 1px solid rgba(239,68,68,0.22); border-radius: 12px; font-size: 13px; color: #FCA5A5; cursor: pointer; font-weight: 600; font-family: 'Plus Jakarta Sans', system-ui, sans-serif; transition: background 0.2s; }
        .browse-clear:hover { background: rgba(239,68,68,0.22); }
        ...
        .item-img-wrap::after { content: ''; position: absolute; inset: 0; background: linear-gradient(to bottom, transparent 60%, rgba(1,30,5,0.05)); pointer-events: none; }
        .item-cat-badge { position: absolute; top: 12px; left: 12px; z-index: 2; background: rgba(255,255,255,0.96); border: 1px solid rgba(4,149,22,0.18); border-radius: 8px; padding: 4px 10px; font-size: 11px; font-weight: 700; color: var(--g-mid); backdrop-filter: blur(8px); transition: all 0.2s; }
        .item-card:hover .item-cat-badge { border-color: rgba(4,149,22,0.3); }
              ...
              {items.map((item: any) => (
                <Link href={`/items/${item.id}`} key={item.id} className="item-card">
                  ...
                  <div className="item-body">
                    ...
                    {item.description && (
                      <p style={{ fontSize: '12px', color: 'var(--tx-muted)', margin: '0 0 14px', lineHeight: '1.6', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any }}>{item.description}</p>
                    )}
